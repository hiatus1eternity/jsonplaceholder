...
def test_albums_get():
    ...
    q = request()
    q.get_simple(gbl.albums_url,gbl.schema_albums_get)
...
def test_albums_element_get(param_albums):
    ...
    q.get_element(url,gbl.schema_albums_element_get,'id',param_albums)
...
def test_albums_phot_get(param_albums):
    ...
    q.get_simple(url1,gbl.schema_albums_phot_get(param_albums))
    q.get_payload(url2,gbl.schema_albums_phot_get(param_albums),payload)
...
def test_albums_user_get(param_users):
    ...
    q.get_payload(url,gbl.schema_albums_user_get(param_users),payload)

def test_albums_post(param_users):
    # post запрос на добавление нового альбома, проверяем при разных userId
    q = request()
    le = q.json_lenght(gbl.albums_url)+1
    payload = {"title":"post_19:55","userId":param_users}
    q.post_simple(gbl.albums_url,gbl.schema_albums_post(param_users,le),
                        payload)
...
def test_albums_put(param_albums,param_users):
    ...
    q.put_simple(url,gbl.schema_albums_post(param_users,param_albums),
                       payload)
...
def test_albums_patch(param_albums):
    ...
    q.patch_simple(url,gbl.schema_albums_patch(param_albums),payload)
